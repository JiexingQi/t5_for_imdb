...
from seq2seq.utils.imdb import imdb_pre_process_function
...
def load_dataset(
    ...
    _imdb_dataset_dict: Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths["imdb"], cache_dir=model_args.cache_dir
    )
    _imdb_metric: Callable[[], Metric] = lambda: datasets.load.load_metric(
        path=data_args.metric_paths["imdb"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )
    _imdb_pre_process_function = lambda batch, max_source_length, max_target_length: imdb_pre_process_function(
        batch=batch,
    ...
    if data_args.dataset == "imdb":
        metric = _imdb_metric()
        dataset_splits = prepare_splits(
            dataset_dict=_imdb_dataset_dict(),
            pre_process_function=_imdb_pre_process_function,
            **_prepare_splits_kwargs,
        )
    else:
